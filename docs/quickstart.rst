...
The self.client attribute makes it possible to make HTTP calls that will be logged by Locust. For more details on how to make other kinds of requests, validate the response, etc, see `Using the HTTP Client <writing-a-locustfile.html#using-the-http-client>`_.
